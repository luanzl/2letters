 ...
 */
async function syncPerfisFromFirestore(emailFilter){
  try{
    if (!window.db) return;

    let query = db.collection(PERFIS_COLLECTION);
    if (emailFilter) {
      query = query.where('emailLower','==', String(emailFilter).toLowerCase());
    }
    ...
    if (remotos.length){
      ...
      localStorage.setItem(LS_KEY, JSON.stringify(state.perfis));
      if (typeof renderPerfis === 'function') renderPerfis();
    } else if ((state.perfis || []).length){
      // Não havia registros remotos, mas há algo local: envia para Firestore para centralizar.
      syncPerfisToFirestore();
    }
  }catch(e){
...
function loadPerfis(){
  ...
  // Sincroniza com o Firestore em background para manter a base de funcionários centralizada
  try {
    syncPerfisFromFirestore();
  } catch(e) {
    console.error(e);
  }
...
async function integrarUsuarioAoSistema(userEmail) {
  ...
  // Garante que a lista de perfis/funcionários esteja sincronizada com o Firestore
  try {
    await syncPerfisFromFirestore(emailLower);
  } catch(e) {
    console.error(e);
  }
  ...
  if (!state.perfis || state.perfis.length === 0) {
    const id = uid();
    state.perfis.push({ id, nome:'👑 Administrador (Proprietário)', email: emailLower, emailLower, senhaHash:'', isAdmin:true });
    savePerfis();
  }
  ...
  if (!perfil) {
    ...
    if (adminSemEmail) {
      ...
      adminSemEmail.emailLower = emailLower;
      savePerfis();
  ...
  // Ao integrar o usuário, sempre buscamos os dados mais recentes deste perfil no Firestore
  if (window.db && typeof syncPerfilFromFirebase === 'function') {
    try {
      await syncPerfilFromFirebase(perfil.id);
    } catch(e) {
      console.error('Erro ao sincronizar dados do perfil a partir do Firestore', e);
    }
  }
